...
interface PhotoToSketchCreatorProps {
    onBack: () => void;
    addImageToLibrary: (imageDataUrl: string) => Promise<void>;
    addHistoryItem: (item: Omit<HistoryItem, 'id'>) => Promise<void>;
}

export const PhotoToSketchCreator: React.FC<PhotoToSketchCreatorProps> = ({ onBack, addImageToLibrary, addHistoryItem }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const sketchPrompt = t('photoToSketchPrompt');
            ...
            if (results.length > 0) {
                ...
                await addHistoryItem({
                    ...
                    sourceImage2: null,
                    referenceImage: null,
                    prompt: `[Photo to Sketch] ${sketchPrompt.substring(0, 50)}...`,
                    negativePrompt: '',
                    imageCount,
        ...
        <div className={`${theme.panelBg} p-5 rounded-xl border ${theme.border} animate-fade-in`}>
            <div className="flex items-center gap-4 mb-8 border-b border-slate-700 pb-4">
                ...
                <div className="flex items-center gap-4">
                    <Icon name="pencil-swoosh" className="w-8 h-8 text-orange-500" />
                    <div>
                        <h2 className={`text-2xl font-bold ${theme.textMain}`}>{t('photoToSketchTitle')}</h2>
                        <p className={`text-sm ${theme.textSub}`}>{t('photoToSketchDesc')}</p>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-4 space-y-6">
                    <section>
                        <div className="flex justify-between items-center mb-3">
                            <h3 className={`font-semibold ${theme.textMain}`}>1. {t('uploadSourceImage')}</h3>
                            <a 
                                href="https://www.pinterest.com/search/pins/?q=modern%20architecture%20photography" 
                                target="_blank" 
                    ...
                    >
                        {isLoading ? <Icon name="sparkles" className="w-5 h-5 animate-spin" /> : <Icon name="pencil-swoosh" className="w-5 h-5" />}
                    ...
                    {isLoading ? (
                       ...
                       <div className="flex flex-col h-full w-full">
                            <div className="flex-grow flex items-center justify-center relative group bg-black/30 rounded-lg overflow-hidden">
                                <img src={selectedImage} alt="Sketch Result" className="max-w-full max-h-[65vh] object-contain shadow-2xl" />
                                <div className="absolute top-4 right-4 flex flex-col gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <a href={selectedImage} download={`sketch-${Date.now()}.png`} className="bg-slate-800/80 backdrop-blur-sm border border-slate-600 hover:bg-slate-700 text-white p-2.5 rounded-full inline-flex" title={t('downloadImage')}>
                        ...
                        <div className={`text-center ${theme.textSub}`}>
                            <Icon name="pencil-swoosh" className="w-16 h-16 mx-auto mb-4 opacity-20" />
