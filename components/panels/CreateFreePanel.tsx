...
export const CreateFreePanel: React.FC<any> = ({
    ...
    onRefreshPrompt, onTabChange, activeTab
}) => {
    const { language, t } = useLanguage();
    const { theme } = useTheme();
    const {
        stylePrompts, contextPrompts, lightingPrompts, ASPECT_RATIO_LABELS
    } = (translations[language] as any).constants;
    
    const [selectedReferenceCategory, setSelectedReferenceCategory] = useState<string>('building');
    const [customSpace, setCustomSpace] = useState(() => localStorage.getItem('aicomplex_custom_space_create') || '');
    const [savedSpaces, setSavedSpaces] = useState<string[]>(() => JSON.parse(localStorage.getItem('aicomplex_saved_spaces_create') || '[]'));
    ...
    useEffect(() => {
        localStorage.setItem('aicomplex_custom_space_create', customSpace);
    }, [customSpace]);

    useEffect(() => {
        localStorage.setItem('aicomplex_saved_spaces_create', JSON.stringify(savedSpaces));
    }, [savedSpaces]);
    ...
    const getPinterestLink = (isReference: boolean = false) => {
        let query = "";
        if (customSpace.trim()) {
            query = `${customSpace.trim()} architecture ${isReference ? 'photography' : 'sketch'}`;
        } else {
            const queries: Record<string, string> = {
                building: "modern architecture building photography",
                house: "modern house design architecture",
                villa: "luxury villa architecture design"
            };
            query = queries[selectedReferenceCategory] || (isReference ? "architecture photography" : "architecture sketch blueprint");
        }
            ...
            <section className="space-y-3 mb-6">
                ...
                <div className="flex bg-[#2a2d33] p-1 rounded-lg border border-white/5">
                    ...
                    <button 
                        ...
                        className={`flex-1 py-2.5 text-[10px] font-black uppercase rounded-lg transition-all ${activeTab === 'planning' ? 'text-orange-500' : 'text-slate-400 hover:text-white'}`}
                    >
                        QUY HOẠCH
                    </button>
                </div>
                ...
                <div className="flex gap-2">
                    {['building', 'house', 'villa'].map((cat) => (
                        <button 
                            key={cat}
                            onClick={() => setSelectedReferenceCategory(cat)}
