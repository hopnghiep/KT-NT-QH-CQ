...
export const CreateApiPanel: React.FC<any> = ({
    ...
    const [customSpace, setCustomSpace] = useState(() => localStorage.getItem('aicomplex_custom_space_api') || '');
    const [savedSpaces, setSavedSpaces] = useState<string[]>(() => JSON.parse(localStorage.getItem('aicomplex_saved_spaces_api') || '[]'));
    ...
    useEffect(() => {
        localStorage.setItem('aicomplex_custom_space_api', customSpace);
    }, [customSpace]);

    useEffect(() => {
        localStorage.setItem('aicomplex_saved_spaces_api', JSON.stringify(savedSpaces));
    }, [savedSpaces]);
